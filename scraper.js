...
async function searchJavtiful(code) {
    ...
    try {
        // Javtiful expects the hyphenated form for codes such as:
        // siro5700 -> siro-5700
        // fc2ppv4949772 -> FC2-PPV-4949772
        const searchQueries = generateSearchQueries(cleanCode);
        ...
        for (const query of searchQueries) {
            ...
            // Support the common Javtiful result URL patterns.
            $('a[href*="/video/"], a[href*="/videos/"], a[href*="/movie/"]').each((i, el) => {
                const href = $(el).attr('href');
                if (!href) return;
                ...
                const slugCode = slug
                    .replace(/-reducing-mosaic.*$/i, '')
                    .replace(/-mosaic.*$/i, '');

                const normSlug = normalizeCode(slug);
                const normSlugCode = normalizeCode(slugCode);
                const normText = normalizeCode(text);

                let score = 0;

                // Exact match is preferred.
                if (normSlug === normalized) score += 200;
                if (normSlugCode === normalized) score += 180;

                // Match against the visible card title/alt text.
                if (normText === normalized) score += 160;
                if (normText.includes(normalized)) score += 60;

                // Also accept a normalized slug containing the requested code.
                if (normSlug.includes(normalized)) score += 80;
            ...
            // Only inspect plausible results. This prevents the first generic
            // video card from being opened when the search page contains many.
            const plausible = candidates.filter(candidate => candidate.score > 0);
            const seen = new Set();
            ...
            for (const candidate of plausible) {
                ...
                console.log(
                    `[Javtiful] Found ${cleanCode} using "${query}" -> ${result.quality || 'unknown'}p`
                );
                return result;
            }
        }
