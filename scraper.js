...
async function searchJavtiful(code) {
    ...
    try {
        const searchUrl =
            `${JAVTIFUL_BASE_URL}/search?q=${encodeURIComponent(cleanCode)}`;
        ...
        $('a[href*="/video/"]').each((i, el) => {
            ...
            const slugCode = slug
                .split('-reducing-mosaic')[0]
                .split('-mosaic')[0];
            const normHay = normalizeCode(`${slug} ${text}`);

            let score = 0;
            if (normalizeCode(slug) === normalized) score += 100;
            if (normalizeCode(slugCode) === normalized) score += 90;
            if (normHay.includes(normalized)) score += 40;
        ...
        for (const candidate of candidates) {
            ...
            console.log(
                `[Javtiful] Found ${cleanCode} -> ${result.quality || 'unknown'}p`
            );
            return result;
        }
    } catch (e) {
